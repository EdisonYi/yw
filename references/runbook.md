...
### RB-41 · OA 跳转薪事力 EHR 异常检查点
- **来源**：运维现场复盘（2026-09-04 用户 OA 跳转薪事力失败，结合 RB-22 运行时白名单 / RB-34 OA 下划线头 / RB-32 反代 / RB-24 Host 允许）。
- **触发**：从 OA 系统点跳转进入薪事力 EHR 后异常——白屏 / 「无法访问」「地址不允许」/「非法请求 ip」/ 跳转后无法登录或又被弹回 OA。
- **🔴 结构化异常检查点清单**（按序逐项核查，任一项不通过即定位到对应根因）：
  1. **确认 OA 集成配置中的薪事力地址**
     - 目的：确认 OA「集成配置 / 单点跳转」条目里填的薪事力目标地址正确、未过期、未被误填成测试/旧地址。
     - 方法：打开 OA 集成配置页 → 找到薪事力单点登录/跳转条目 → 核对 URL（协议 http/https、域名、端口、上下文路径）。
     - 通过标准：与当前薪事力实际部署地址**完全一致**（含端口；走 nginx 反代则是反代入口地址）。
     - 失败定位：地址错/旧/缺端口 → 改 OA 集成配置为目标地址重试（OA 侧配置，非 yw 直接处置）。
  2. **确认该薪事力地址可单独正常访问并成功登录薪事力系统**
     - 目的：排除薪事力后端自身异常（应用未起 / 库挂 / 证书错 / Host 未放行）。
     - 方法：浏览器**直接**访问第 1 项确认的地址 → 走完整登录流程（输账号密码登录成功、能进首页业务）。
     - 通过标准：单点直访可登录、业务页正常；`ss -tlnp` 端口在监听、`tail -200f /usr/local/dhr/logs/dhr.log` 无新 ERROR（RB-11）。
     - 失败定位：直访都不行 → 后端问题，回 RB-11 分层定位（进程/端口/日志）；证书告警 → RB-19；Host 不允许 → RB-24 `ours.security.hosts`；报「非法请求 ip」→ RB-22 白名单。
  3. **确认 OA 服务器与本地 PC 网络环境均能正常访问该薪事力地址**
     - 目的：确认两侧网络链路都通，避免「OA 服务器出网受限 / 本地 PC 被防火墙拦 / DNS 解析不了」导致跳转失败。
     - 方法：① 在 **OA 服务器**上 `curl -sv <地址>/health`（或首页）确认 200；② 在**本地 PC**（用户实际操作机）浏览器/命令行访问同一地址确认可达。
     - 通过标准：OA 服务器侧与本地 PC 侧**两处**都能解析 + 建连 + 返回 200（或登录页）。
     - 失败定位：仅 OA 服务器不通 → 查其出网白名单/代理/防火墙；仅本地 PC 不通 → 查本地网络/浏览器/DNS/Hosts；两处都不通 → 地址本身不可达（回到第 1/2 项）。
- **关联根因速查**（检查点全过仍跳转异常，按此精确定位）：
  - OA 表单推送报 `dev_token 缺失` → nginx 未开 `underscores_in_headers on`（RB-34）；
  - 进薪事力后智多薪菜单「非法请求 ip」→ IP 白名单漏 **nginx 本机 IP**（RB-22，双入口场景见 RB-32/RB-34）；
  - 反代 502 / 连接被拒 → `proxy_pass` 协议与后端不一致（RB-32 坑①）；
  - 后端 302 跳回 http 明文 → 外网反代漏 `proxy_redirect http:// https://`（RB-32）。
- **闭环验证**：检查点 ①→②→③ 全过 + 关联根因无命中 → 从 OA 点跳转可正常进入并登录薪事力；否则按对应 RB 处置。
