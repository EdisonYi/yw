...
> 本 Skill 的 `runbook.md`（RB-10~RB-41）与 `work-log.md` 提炼自下列**官方部署文档**与**语雀导出的运维知识**。
> 原始文件存于用户本机 `Downloads/`，**不纳入本仓库版本管理**（避免 PDF/DOCX 二进制膨胀）；此处仅做溯源归类，便于回溯与补证。
> 最后整理：2026-09-03（七批：无运维服务器升级步骤 → 回写 RB-18 路径A 备份配置文件步骤；RB-10~RB-41）。
> 2026-09-04 增补 RB-41（OA 跳转薪事力 EHR 异常检查点，来源为运维现场 OA 跳转失败复盘，非语雀文档；关联 RB-22/34/32/24）。
